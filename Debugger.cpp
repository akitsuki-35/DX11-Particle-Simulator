...
const void Debugger::Initialize(HWND hwnd) const
{
	...
	// DirectX11用の初期化
	ImGui_ImplDX11_Initialize(Renderer::GetDevice(), Renderer::GetDeviceContext());
}
...
{
	BezierCurve* b = Game::GetGameObject<ParticleBezier>()->GetBezier();
	if (!b) return;
	...
	ImGui::SliderFloat("[0].X", &b->mControlPoint[0].position.x, -30.0f, 30.0f, "%.2f");
	ImGui::SliderFloat("[0].Y", &b->mControlPoint[0].position.y, -10.0f, 10.0f, "%.2f");
	ImGui::SliderFloat("[0].Z", &b->mControlPoint[0].position.z, -30.0f, 30.0f, "%.2f");

	ImGui::SeparatorText("ControlPoint[1]");
	ImGui::SliderFloat("[1].X", &b->mControlPoint[1].position.x, -30.0f, 30.0f, "%.2f");
	ImGui::SliderFloat("[1].Y", &b->mControlPoint[1].position.y, -10.0f, 10.0f, "%.2f");
	ImGui::SliderFloat("[1].Z", &b->mControlPoint[1].position.z, -30.0f, 30.0f, "%.2f");

	ImGui::SeparatorText("ControlPoint[2]");
	ImGui::SliderFloat("[2].X", &b->mControlPoint[2].position.x, -30.0f, 30.0f, "%.2f");
	ImGui::SliderFloat("[2].Y", &b->mControlPoint[2].position.y, -10.0f, 10.0f, "%.2f");
	ImGui::SliderFloat("[2].Z", &b->mControlPoint[2].position.z, -30.0f, 30.0f, "%.2f");
	ImGui::Separator();

	ImGui::SeparatorText("ControlPoint[3]");
	ImGui::SliderFloat("[3].X", &b->mControlPoint[3].position.x, -30.0f, 30.0f, "%.2f");
	ImGui::SliderFloat("[3].Y", &b->mControlPoint[3].position.y, -10.0f, 10.0f, "%.2f");
	ImGui::SliderFloat("[3].Z", &b->mControlPoint[3].position.z, -30.0f, 30.0f, "%.2f");
...
{
	ParticleBezier* p = Game::GetGameObject<ParticleBezier>();
	if (!p) return;
